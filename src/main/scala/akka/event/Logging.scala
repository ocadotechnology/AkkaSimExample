...
/**
  * TODO: This is actually just a copy of Logging.scala from the Akka codebase.
  * We need to modify that instead, but that means making it compile and build locally for testing,
  * which I haven't had a chance to do yet.
  *
  * Changes from the original are solely in addLogger to not use `Await` (i.e. ignore failures to initialise logging).
  */
